...
function getCart()
{
    ...
    if (isset($_SESSION['saveCart']['Total'])) {
        ...
        $html = "
            <tr>
                <td colspan='3' style='display:flex; justify-content:space-between; width:182%; border-top:1px solid #ccc;'><p>Total<p><p>{$Total} đ</p></td>
            </tr>";
        echo $html;
    }
}
function SubmitThanhToan()
{
    ...
    if (
        ...
        $query1->bind_param("ssssssssi", $ngdh, $name, $phone, $email, $address, $tinh, $quan, $xa, $thanhtoan);
        $ok = $query1->execute();
            ...
            if ($query) {
                if ($query->num_rows > 0) {
                    ...
                    foreach ($_SESSION["cart"] as $id => $value) {
                        ...
                        $query->bind_param("isii", $row[0], $id, $value['num'], $Total);
                        $ok = $query->execute();
                    }
                    echo "Thành công";
                }
